  ...
  return (
    <div className="space-y-6 pb-6">
      <div className="grid grid-cols-2 rounded-2xl bg-white/[0.04] p-1">
        <button onClick={() => updateCheckIn({ mode: "place" })} className={`rounded-xl px-3 py-3 text-sm font-semibold transition ${checkIn.mode === "place" ? "bg-white text-black" : "text-zinc-500"}`}>Attività</button>
        <button onClick={() => updateCheckIn({ mode: "free" })} className={`rounded-xl px-3 py-3 text-sm font-semibold transition ${checkIn.mode === "free" ? "bg-white text-black" : "text-zinc-500"}`}>Luogo libero</button>
      </div>

      {checkIn.mode === "place" ? <div><label className="mb-2 block text-xs font-bold uppercase tracking-wider text-zinc-500">Dove sei?</label><div className="relative"><SearchIcon className="absolute left-4 top-1/2 size-5 -translate-y-1/2 text-zinc-500" /><input className="w-full rounded-2xl border border-white/10 bg-panel py-4 pl-12 pr-4 text-sm outline-none placeholder:text-zinc-600 focus:border-violet/50" placeholder="Cerca un’attività registrata" /></div><div className="mt-3 space-y-2">{places.slice(0, 3).map((place) => <button key={place.id} onClick={() => updateCheckIn({ selectedPlace: place.id })} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left ${checkIn.selectedPlace === place.id ? "border-violet/50 bg-violet/[0.07]" : "border-white/10 bg-panel"}`}><span className="grid size-11 place-items-center rounded-xl bg-white/[0.06]"><PinIcon className="size-5 text-violet" /></span><span className="flex-1"><span className="block text-sm font-semibold">{place.name}</span><span className="text-xs text-zinc-500">{place.category} · {place.distance}</span></span>{checkIn.selectedPlace === place.id && <CheckIcon className="size-5 text-violet" />}</button>)}</div></div> : <div className="space-y-3"><label className="block text-xs font-bold uppercase tracking-wider text-zinc-500">Descrivi il luogo</label><input value={checkIn.freePlace} onChange={(event) => updateCheckIn({ freePlace: event.target.value })} className="w-full rounded-2xl border border-white/10 bg-panel px-4 py-4 text-sm outline-none focus:border-violet/50" placeholder="Es. Al parco vicino alle terme" /><input value={checkIn.mapsUrl} onChange={(event) => updateCheckIn({ mapsUrl: event.target.value })} className="w-full rounded-2xl border border-white/10 bg-panel px-4 py-4 text-sm outline-none focus:border-violet/50" placeholder="Link Google Maps (facoltativo)" /></div>}

      <fieldset><legend className="mb-3 text-xs font-bold uppercase tracking-wider text-zinc-500">Chi può vederlo?</legend><div className="space-y-2">{["Amici", ...groups.slice(0, 2).map((group) => group.name)].map((item) => <button key={item} onClick={() => updateCheckIn({ visibility: item })} className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-panel p-4 text-sm font-semibold"><span>{item === "Amici" ? "Tutti gli amici" : item}</span><span className={`grid size-5 place-items-center rounded-full border ${checkIn.visibility === item ? "border-violet bg-violet text-white" : "border-zinc-600"}`}>{checkIn.visibility === item && <CheckIcon className="size-3" />}</span></button>)}</div></fieldset>
