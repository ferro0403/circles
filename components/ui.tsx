    ...
    <header className="flex items-center justify-between gap-4 py-4">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          {eyebrow && <p className="mb-0.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-lime">{eyebrow}</p>}
...
export function SectionHeader({ title, href, label = "Vedi tutto" }: { title: string; href?: string; label?: string }) {
  return <div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-bold tracking-tight text-white">{title}</h2>{href && <Link href={href} className="flex items-center text-xs font-semibold text-zinc-400 hover:text-lime">{label}<ChevronRightIcon className="size-4" /></Link>}</div>;
}
...
export function Pill({ children, active = false }: { children: ReactNode; active?: boolean }) {
  return <span className={`inline-flex shrink-0 items-center rounded-full border px-3 py-2 text-xs font-semibold ${active ? "border-lime bg-lime text-black" : "border-white/10 bg-white/[0.04] text-zinc-300"}`}>{children}</span>;
}

export function StatusDot({ className = "" }: { className?: string }) {
  return <span className={`inline-block size-2 rounded-full bg-lime shadow-[0_0_12px_rgba(201,255,74,.7)] ${className}`} />;
}
