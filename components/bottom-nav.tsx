    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto max-w-md border-t border-white/10 bg-[#0b0c0e]/95 px-3 pb-[max(0.65rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl">
      <div className="flex items-end justify-between">
        {items.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} aria-label={item.label} className={`group flex min-w-14 flex-col items-center gap-1 text-[10px] font-medium transition ${active ? "text-lime" : "text-zinc-500 hover:text-zinc-200"}`}>
              <span className={item.primary ? "-mt-7 grid size-14 place-items-center rounded-full border-4 border-[#0b0c0e] bg-lime text-black shadow-glow transition group-active:scale-95" : "grid size-7 place-items-center"}>
