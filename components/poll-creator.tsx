...
export function PollCreator({ groupId, initialTitle, initialVotes = 0, initialCloses }: { groupId: string; initialTitle?: string; initialVotes?: number; initialCloses?: string }) {
  ...
  return <section><div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-violet">Decidete insieme</p><h2 className="mt-0.5 text-lg font-bold">Sondaggi</h2></div><button onClick={() => setCreating((value) => !value)} className="flex items-center gap-1.5 rounded-full bg-violet px-3.5 py-2.5 text-xs font-bold text-white shadow-glow"><PlusIcon className="size-4" />Nuovo sondaggio</button></div>
    {creating && <form onSubmit={createPoll} className="mb-3 rounded-3xl border border-violet/30 bg-gradient-to-br from-violet/[0.12] to-panel p-5">
      ...
      <div className="mt-4 grid grid-cols-2 items-start gap-3"><label className="flex min-w-0 flex-col items-center text-center text-xs font-bold text-zinc-300">Data proposta<input required type="date" min={today()} value={proposedDate} onChange={(event) => setProposedDate(event.target.value)} className="centered-date-time mt-2 h-12 w-full min-w-0 rounded-xl border border-white/10 bg-black/30 px-2 text-center text-sm font-normal outline-none focus:border-violet" /></label><label className="flex min-w-0 flex-col items-center text-center text-xs font-bold text-zinc-300">Ora proposta<input required type="time" value={proposedTime} onChange={(event) => setProposedTime(event.target.value)} className="centered-date-time mt-2 h-12 w-full min-w-0 rounded-xl border border-white/10 bg-black/30 px-2 text-center text-sm font-normal outline-none focus:border-violet" /></label></div>
      ...
      <fieldset className="mt-4"><legend className="text-xs font-bold text-zinc-300">Opzioni del sondaggio</legend><div className="mt-2 space-y-3">{options.map((option, index) => <div key={index} className="grid min-w-0 grid-cols-[5.25rem_minmax(0,1fr)] gap-2"><select aria-label={`Tipo opzione ${index + 1}`} value={option.source} onChange={(event) => updateOption(index, { source: event.target.value as OptionDraft["source"], value: event.target.value === "place" ? places[0]?.id ?? "" : "" })} className="min-w-0 rounded-xl border border-white/10 bg-black/30 px-2 text-xs outline-none focus:border-violet"><option value="place">Attività</option><option value="free">Libero</option></select>{option.source === "place" ? <select aria-label={`Opzione ${index + 1}`} value={option.value} onChange={(event) => updateOption(index, { value: event.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-3 text-sm outline-none focus:border-violet">{places.map((place) => <option key={place.id} value={place.id}>{place.name}</option>)}</select> : <input aria-label={`Opzione ${index + 1}`} required value={option.value} onChange={(event) => updateOption(index, { value: event.target.value })} className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-3 text-sm outline-none focus:border-violet" placeholder={`Luogo/opzione ${index + 1}`} />}</div>)}</div></fieldset>
    ...
    <div className="space-y-3">{polls.map((poll) => {
      ...
      return <article key={poll.id} className={`rounded-3xl border p-5 ${poll.isMock ? "border-white/10 bg-panel" : "border-violet/30 bg-violet/[0.08]"}`}><span className="rounded-full bg-violet/10 px-2.5 py-1 text-[10px] font-bold text-violet">{poll.isMock ? "SCELTA DI GRUPPO" : "SONDAGGIO PUBBLICATO"}</span><h3 className="mt-4 text-lg font-bold">{poll.title}</h3>{!poll.isMock && <div className="mt-2 space-y-1 text-xs text-zinc-500"><p className="flex items-center gap-1.5"><CalendarIcon className="size-3.5" />{formatPollDate(poll.proposedDate)} <ClockIcon className="ml-2 size-3.5" />{poll.proposedTime}</p><p className="flex items-center gap-1.5"><PinIcon className="size-3.5" />{poll.proposedLocation}</p></div>}<div className="mt-4 space-y-2">{poll.options.map((option, index) => {
        ...
        return <button key={option.id} type="button" onClick={() => vote(poll.id, option.id)} aria-pressed={isSelected} className={`relative flex w-full overflow-hidden rounded-xl border px-3 py-3 text-left text-xs font-semibold ${isSelected ? "border-violet bg-violet/[0.18] text-white shadow-[0_0_0_1px_rgba(139,92,246,0.2)]" : "border-white/10 bg-white/[0.05]"}`}><span className="absolute inset-y-0 left-0 bg-violet/10" style={{ width: `${percent}%` }} /><span className="relative flex-1">{option.label}</span><span className="relative flex items-center gap-1 text-zinc-400">{isSelected && <CheckIcon className="size-3 text-violet" />}{count} {count === 1 ? "voto" : "voti"}</span></button>;
