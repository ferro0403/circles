...
export default async function PlaceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="pb-4"><PageHeader title={place.name} eyebrow={place.category} backHref="/esplora" />
    ...
    <div className="mt-5 flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className={`size-2 rounded-full ${place.openNow ? "bg-violet" : "bg-zinc-600"}`} /><span className={`text-xs font-semibold ${place.openNow ? "text-violet" : "text-zinc-500"}`}>{place.openNow ? "Aperto ora" : "Chiuso ora"}</span></div><p className="mt-2 flex items-start gap-1.5 text-sm leading-5 text-zinc-400"><PinIcon className="mt-0.5 size-4 shrink-0" />{place.address}</p></div><span className="rounded-full border border-white/10 px-3 py-2 text-xs text-zinc-400">{place.distance}</span></div>
    ...
    <section className="mt-8 rounded-3xl border border-white/10 bg-panel p-5"><div className="flex items-center justify-between"><div><h2 className="font-bold">Amici qui adesso</h2><p className="mt-1 text-xs text-zinc-500">Mostriamo solo persone che conosci</p></div><span className="flex items-center gap-1 rounded-full bg-violet/10 px-2.5 py-1 text-xs font-bold text-violet"><StatusDot />{place.friends.length}</span></div><div className="mt-5 space-y-4">{place.friends.map((friend, index) => <div key={friend} className="flex items-center gap-3"><Avatar initials={friend.slice(0, 2).toUpperCase()} className="size-11" tone={index % 2 ? "bg-cyan-300" : "bg-violet-300"} /><div className="flex-1"><p className="text-sm font-bold">{friend}</p><p className="text-xs text-zinc-500">Qui da circa {index + 1}h</p></div><Link href="/check-in" className="rounded-full border border-white/10 px-3 py-2 text-xs font-semibold">Raggiungi</Link></div>)}</div></section>
  </div>;
}
