...
export default function ProfilePage() {
  return <div className="pb-4"><PageHeader title="Profilo" eyebrow="Il tuo spazio" />
    <section className="mt-4 flex flex-col items-center rounded-[2rem] border border-white/10 bg-panel p-6 text-center"><div className="relative"><Avatar initials={currentUser.initials} className="size-24 text-2xl" tone="bg-lime" /><span className="absolute bottom-1 right-1 size-4 rounded-full border-4 border-panel bg-lime" /></div><h2 className="mt-4 text-2xl font-bold">{currentUser.name}</h2><p className="mt-1 text-sm font-semibold text-lime">{currentUser.nickname}</p><p className="mt-3 max-w-xs text-sm text-zinc-500">Riolo Terme · Nel gruppo dal 2026</p><button className="mt-5 rounded-full border border-white/10 px-5 py-2.5 text-xs font-bold">Modifica profilo</button></section>
    ...
    <section className="mt-8"><div className="mb-3 flex items-center gap-2"><ShieldIcon className="size-4 text-lime" /><h2 className="text-lg font-bold">Privacy e impostazioni</h2></div><div className="overflow-hidden rounded-3xl border border-white/10 bg-panel">{settings.map((setting, index) => { const Icon = setting.icon; return <button key={setting.title} className={`flex w-full items-center gap-3 p-4 text-left ${index ? "border-t border-white/[0.07]" : ""}`}><span className="grid size-10 place-items-center rounded-xl bg-white/[0.05]"><Icon className="size-4 text-lime" /></span><span className="flex-1"><span className="block text-sm font-semibold">{setting.title}</span><span className="mt-0.5 block text-xs text-zinc-500">{setting.value}</span></span><ChevronRightIcon className="size-5 text-zinc-600" /></button>; })}</div></section>
